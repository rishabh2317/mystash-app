...
export default function PersonalReelsScreen() {
  ...
  const load = useCallback(async (id: string, nextCursor?: string | null, append = false) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : PERSONAL_PROFILE_COPY.emptyReels);
    } finally {
      setLoading(false);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: tokens.color.canvas }}>
      <TopBar mode="page" title={PERSONAL_PROFILE_COPY.reels} showBack showBag={false} />
      <PersonalCollectionList
        items={items}
        loading={loading}
        error={error}
        empty={PERSONAL_PROFILE_COPY.emptyReels}
        variant="reel"
